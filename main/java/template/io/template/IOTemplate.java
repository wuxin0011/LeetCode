...
public class IOTemplate {
    ...
    public static class IO {
        static int MAXN_N = (int) 1e6 + 1;

        public static final String DEFAULT_FILE_NAME = "../in/in.txt"; // default read file
        ...
        public  String[] readStringArray(){
            if( STRING_ARRAY == null) {
                STRING_ARRAY = new String[MAXN_N];
            }
            int size = 0;
            while(true) {
                try{
                    if (CHAR_ARRAY == null) {
                        CHAR_ARRAY = new char[MAXN_N];
                    }
                    int x = br.read();
                    if(x == -1){
                        return null;
